...
var nonEnumerablePropertySymbols = require( './../../dist' ); // eslint-disable-line id-length
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof nonEnumerablePropertySymbols, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function returns an array of an object\'s own non-enumerable symbol properties', function test( t ) {
	var hasSymbols;
	var expected;
	var actual;
	var syms;
	...
	if ( hasSymbols ) {
		...
		expected = [ syms[ 2 ], syms[ 3 ], syms[ 4 ] ];
	} else {
		syms = [];
		expected = [];
	}
	actual = nonEnumerablePropertySymbols( obj );
...
tape( 'the function returns an array of an object\'s own non-enumerable symbol properties (mock)', function test( t ) {
	var nonEnumerablePropertySymbols; // eslint-disable-line id-length
	var expected;
	var actual;
	var syms;
	var obj;
	...
	nonEnumerablePropertySymbols = proxyquire( './../dist/main.js', {
...
tape( 'the function returns an empty array if provided `null` or `undefined`', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		actual = nonEnumerablePropertySymbols( values[ i ] );
